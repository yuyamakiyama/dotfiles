...
prompt_pure_preprompt_render() {
	...
	# Use the stored base prompt directly to avoid accumulation issues
	# caused by fragile stripping logic during async callback re-renders.
	local -ah ps1
	ps1=(
		${(j. .)preprompt_parts}  # Join parts, space separated.
		$prompt_newline           # Separate preprompt and prompt.
		$prompt_pure_base_prompt
	)
...
prompt_pure_setup() {
	...
	# Store the base prompt so preprompt_render can use it directly
	# instead of trying to strip the preprompt from PROMPT each time.
	typeset -g prompt_pure_base_prompt=$PROMPT
